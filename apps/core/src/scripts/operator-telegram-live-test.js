...
import { TELEGRAM_SUBJECT } from '../publisher/platforms/telegram/constants.js';

const db = getDb();
const id = Number(process.argv.find(arg => arg.startsWith('--target='))?.split('=')[1]);

if (process.env.TELEGRAM_LIVE_TEST !== 'true' || !Number.isInteger(id) || id <= 0) {
  console.error('Refusing: set TELEGRAM_LIVE_TEST=true and pass --target=<CampaignTarget ID>.');
  process.exitCode = 1;
} else {
  try {
    const job = await db.transaction(async trx => {
      const target = await trx('campaign_targets').where({ id }).forUpdate().first();
      if (!target || !['telegram', 'telegram_channel'].includes(target.platform)) {
        throw new Error('Target is not a Telegram target');
      }
      ...
      if (!target.integration_config_id) {
        throw new Error('Telegram target is missing active IntegrationConfig connection');
      }
      ...
      if (!config) {
        throw new Error('Active Telegram IntegrationConfig is missing or not tenant-owned');
      }
      ...
      const [jobId] = await trx('publish_jobs').insert({
        ...
        idempotency_key: `live-tg-c${campaign.id}-t${id}`,
        status: 'queued',
        attempt_count: 0,
        max_attempts: 3,
    ...
    console.log(`Queued Telegram PublishJob ${job.id} on ${TELEGRAM_SUBJECT}; no Telegram Bot API was called directly.`);
    console.log(`Target ID: ${id}`);
    console.log(`Run worker to execute publish: pnpm --filter @esima/core publisher:telegram:worker`);
  } catch (error) {
